...
class ScrapDateRange(models.AbstractModel):
    _name = 'report.dev_scrap_report.template_scrap_date_range'
    ...
    def get_scrap_data(self, o):
        print
        data = []
        start_date = self.timezone_conversion(o.start_date)
        end_date = self.timezone_conversion(o.end_date)
        if start_date and end_date:
            scrap_ids = self.env['stock.scrap'].search([('date_done', '>=', str(start_date)),
    ...
    def _get_report_values(self, docids, data=None):
        docs = self.env['scrap.date.range.report'].browse(docids)
        return {'doc_ids': docids,
                'doc_model': 'scrap.date.range.report',
                'docs': docs,
