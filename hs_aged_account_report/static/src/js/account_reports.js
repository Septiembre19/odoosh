odoo.define('aged_account_report.account_report', function (require) {
	'use strict';
	...
	AccountReportsWidget.include({
		...
		AccountReportsWidget.prototype.custom_events, {
			'value_changed': function(ev) {
				var self = this;
				self.report_options.account_account_ids = ev.data.account_account_ids;
		...
		get_account_field: function(){
			return {
				...
				value: this.report_options.account_account_ids.map(Number),
				domain: this.report_options.domain_account_ids.map(Number),
			};
		},
		...
		render_searchview_buttons: function() {
			...
			if (this.report_options.account_account) {
				if (!this.M2MAccountFilter) {
					var fields = {};
					if (this.report_options.account_account_ids) {
						fields['account_account_ids'] = {
							label: _t('Accounts'),
							modelName: 'account.account',
							value: this.report_options.account_account_ids.map(Number),
							domain: this.report_options.domain_account_ids.map(Number),
					...
					if (!_.isEmpty(fields)) {
						this.M2MAccountFilter = new M2MFilters(this, fields);
						this.M2MAccountFilter.appendTo(this.$searchview_buttons.find('.js_account_account_m2m'));
					}
				}else {
					this.$searchview_buttons.find('.js_account_account_m2m').append(this.M2MAccountFilter.$el);
				}
			}
		}
	});
