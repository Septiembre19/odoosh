...
class SaleReportAdvance(models.TransientModel):
    ...
    customer_ids = fields.Many2many('res.partner', string="Cliente",required=True)
    salesperson_ids = fields.Many2many('res.users', string="Vendedor", required=True)
    from_date = fields.Date(string="Fecha Inicio")
    to_date = fields.Date(string="Fecha Fin")
    status = fields.Selection([('open', 'Abierto'), ('paid', 'Pagado'), ('both', 'Ambos')],
                              string='Estado de la Factura', default='open', reqired=True)
    company_ids = fields.Many2many('res.company', string='Compañía')
    ...
    def _get_data(self):
        ...
        result = []
        salesperson = []
        for rec in self.salesperson_ids:
            a = {
                'id': rec,
                'name': rec.name
            }
            salesperson.append(a)
        for so in sales_order:
            for cust in salesperson:
                if cust['id'] == so['user_id']:
                    if so.invoice_ids:
                        if self.status == 'open':
                            for inv in so.invoice_ids:
                                if inv.payment_state != 'paid':
                                    res = {
                                                ...
                                                'partner_name_ext': so.partner_name_ext,
                                                'user_id': so.user_id,
                                                'order_date': so.date_order,
                                                'type': inv.move_type,
                                                'invoice': inv.name,
                                                'date': inv.invoice_date,
                                                'invoiced': inv.amount_untaxed,
                            ...
                            for inv in so.invoice_ids:
                                if inv.payment_state == 'paid':
                                    ...
                                    if inv.payment_state == 'paid' and inv.move_type == 'out_refund':
                                        res = {
                                                    'so': so.name,
                                                    'partner_id': so.partner_id.name,
                                                    'partner_name_ext': so.partner_name_ext,
